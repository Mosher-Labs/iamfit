...
1. Once the CLI has a rough working diff for a handful of resource types: get
   at least one person outside this conversation to run it against real (or
   realistic) Terraform and react.
1. Track whether anyone would actually install/use it before building the
   paid tier — building monetization before any free-tier pull is a common
   trap worth avoiding.
1. Revisit this doc after each validation step and update ASSUMED -> VALIDATED
